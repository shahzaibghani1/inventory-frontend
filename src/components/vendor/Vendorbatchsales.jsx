...
import Vendorcreatesales from './vendorcreatesales';
import Vendornavbar from './Vendornavbar';


function VendorBatchSales() {
    const batchId = 0;
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/vi/vendor/vendorBatchSales/findByBatchName/${batchId}`, { headers });
  ...
  const handleSaveClick = async (batchId) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `${process.env.REACT_APP_BASE_URL}/api/vi/vendor/vendorBatchSales/getByIdAndUpdate/${batchId}`,
        updatedBatchSales,
        { headers }
      );
      if(response)
  ...
  const handleDeleteClick = async (batchId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_BASE_URL}/api/vi/vendor/vendorBatchSales/getByIdAndDelete/${batchId}`,
        { headers }
      );
  ...
  return (
    <div>
      ...
      <h3 onClick={toggleCreateForm}>Don't have any BatchSales? Create Batch Sales? Click here</h3>
      {showCreateForm && <Vendorcreatesales userId= {convert.userId}onClose={handleCreateFormClose} />}
      <ToastContainer />
    </div>
  );
}

export default VendorBatchSales;
